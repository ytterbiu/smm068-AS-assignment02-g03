...
## Read raw price data from submitted workbook 
# Column B has closing prices; data starts at row 9 (row 8 is the header)
price_data <- read_excel(
  "CM2_cwk2_draft.xlsm",
  sheet = "Sheet1",
  range = cell_limits(c(9, 2), c(NA, 2)),
  col_names = "close"
)

# Drop any trailing NA rows
price_data <- price_data %>% filter(!is.na(close))
...
$$
\widehat{\text{Var}}(r_t) = `r formatC(var_daily, format = "fg", digits = 6)`
$$
